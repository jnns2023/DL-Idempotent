...
def main(testing=True):
    ...
    hparams = {
        ...
        'beta2': 0.999,
        'n_epochs': 100,
        'momentum': 0.1, # default is 0.1
        'layer_norm': True, # deviation from original approach, default is False
        'batch_size': 256,
        'testing': testing, 
        'save_path': save_path, 
        'save_interval': 10,
    ...
    f = IdemNetMnist(momentum=hparams['momentum'], layer_norm=hparams['layer_norm']).to(device)
    f_copy = IdemNetMnist(momentum=hparams['momentum'], layer_norm=hparams['layer_norm']).to(device)
