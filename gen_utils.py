...
def generate_frequency_noise(batch_data):
    """
    ...
    real_mean = fft_data.real.mean(dim=0, keepdim=True)
    real_std = fft_data.real.std(dim=0, keepdim=True)
    imag_mean = fft_data.imag.mean(dim=0, keepdim=True)
    imag_std = fft_data.imag.std(dim=0, keepdim=True)
    
    # Sample noise in the frequency domain using the computed statistics
    noise_real = torch.normal(real_mean, real_std)
    noise_imag = torch.normal(imag_mean, imag_std)
    
    # Combine real and imaginary parts to form complex noise
    noise_frequency = torch.complex(noise_real, noise_imag)
